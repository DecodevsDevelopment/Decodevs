        ...
        <Dialog.Panel className='fixed inset-y-0 h-2/4 right-0 z-50 w-full overflow-y-auto bg-gray-700 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10'>
          ...
          <div className='mt-6 flow-root'>
            <div className='-my-6 divide-y divide-gray-500/10'>
              <div className='space-y-2 py-6 flex text-center flex-col justify-center'>
                {/* Elementos de navegación */}
                {navigation.map((item) => (
                  <Link key={item.name} to={item.path} className='-mx-3 block rounded-lg px-3 py-2 text-base navbar font-semibold leading-7 text-white'>
                    {item.name}
                  </Link>
                ))}
              </div>
